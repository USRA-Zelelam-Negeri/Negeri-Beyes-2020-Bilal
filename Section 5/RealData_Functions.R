...
#--------------------------------------------------------------------------------------------
# The "fit_bbn_old()" function is used to fit the BBN model of Chu and Cole (2006)
#--------------------------------------------------------------------------------------------
fit_bbn_old <- function(Data, Study){
...
#--------------------------------------------------------------------------------------------
# The "fit_bbn()" function is used to fit the BBN model of Chu and Cole (2006) using the 
...
fit_bbn <- function(Data, Study){
  ...
  # Now fit the BBN RE model of Chu et al. (2006)
  # Only change from fit_bbn_old(), using glmer() instead
  ##################################
  groupS<-data.frame(Study, wellclassified=Data$TP, malclassified=Data$FN, group="disease" )
  
  groupH<-data.frame(Study, wellclassified=Data$TN, malclassified=Data$FP, group="healthy"  )
  
  X_long<-rbind(groupS,groupH)
  
  glmer_bbn <- glmer(cbind(wellclassified,malclassified) ~ 
                      group - 1 + (group -1|Study),
                      family=binomial,
                      data=X_long)
  
  BIC <- BIC(glmer_bbn)
  AIC <- AIC(glmer_bbn)
  deviance <- deviance(glmer_bbn)
  logLik <- logLik(glmer_bbn)
  fixef <- fixef(glmer_bbn)
  ranef <- ranef(glmer_bbn)
  resid <- resid(glmer_bbn)
  VarCorr <- VarCorr(glmer_bbn)
  anova <- anova(glmer_bbn)
  coef <- coef(glmer_bbn)
  vcov <- vcov(glmer_bbn)  
  rancoef <- summary(glmer_bbn)$varcor$Study
  ...
  # print(vcov)
  # print('summary')
  # print(summary(glmer_bbn))
  # print('summary$varcor')
  # print(summary(glmer_bbn)$varcor)
  # print('summary$varcor$study')
  # print(summary(glmer_bbn)$varcor$Study)
  # print('rancoef')
  # print(rancoef)
  # 
  ...
  fit.bbn$metatron.bbn <- glmer_bbn
  fit.bbn$SeSp <- ilogit(as.numeric(fixef))
  fit.bbn$SeSp.logit <- as.numeric(fixef)
  fit.bbn$Psi <- rancoef[1:2,1:2]
  fit.bbn$vcov <- vcov
  fit.bbn$CIs <- c("Se.lb"=ilogit(fit.bbn$SeSp.logit[1]-qt(p=0.975, df=k-2)*sqrt(fit.bbn$vcov[1,1])), 
                   "Se.ub"=ilogit(fit.bbn$SeSp.logit[1]+qt(p=0.975, df=k-2)*sqrt(fit.bbn$vcov[1,1])),
                   "Sp.lb"=ilogit(fit.bbn$SeSp.logit[2]-qt(p=0.975, df=k-2)*sqrt(fit.bbn$vcov[2,2])), 
                   "Sp.ub"=ilogit(fit.bbn$SeSp.logit[2]+qt(p=0.975, df=k-2)*sqrt(fit.bbn$vcov[2,2])))
  fit.bbn$Stats <- c(logLik=loglik.bbn, AIC= -2*loglik.bbn + 2*5, BIC = -2*loglik.bbn + 5*log(k*2))
...
# R value too.
#----------------------------------------------------------------------------------
mcemDTA <- function(Data, outl, mu0=c(0,0), sigma0=c(0,0,0), R=10, delta = 0.001, eps = 0.005, maxit=150){
  # Data is a dataframe or matrix of row k consisting of the 2x2 DTA data 
  # outl is a vector consisting of the study number identified as potentially outlying or influential
  ...
  # Set starting values
  if(all(mu0==0)){
    fit0 <- fit_bnn(Data = Data[-outl,], method = "ml")
    mu0 <- fit0$SeSp.logit 
  }
